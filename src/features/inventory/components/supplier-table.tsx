  ...
  if (suppliers.length === 0) {
    return (
      <div className="bg-white rounded-xl border border-border p-12 text-center">
        ...
        <p className="text-sm text-gray-500">Klik &quot;Tambah Supplier&quot; untuk menambahkan supplier baru</p>
      </div>
    );
  }
      ...
      <Table>
        <TableHeader>
          <TableRow className="bg-gray-50/50">
            ...
            <TableHead className="cursor-pointer hover:bg-gray-100 transition-colors text-center" onClick={() => handleSort('materialCount')}>
            ...
            <TableRow key={supplier.id} className="hover:bg-gray-50/50">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    {supplier.paymentTerm && (
                      <span className="block text-xs text-gray-400">Tempo: {supplier.paymentTerm}</span>
                    )}
                  </div>
                </div>
              </TableCell>
              <TableCell className="text-sm text-gray-600">{supplier.phone || '-'}</TableCell>
              <TableCell className="text-sm text-gray-500 max-w-[200px] truncate">{supplier.address || '-'}</TableCell>
              ...
              <TableCell>
                <div className="flex items-center justify-center gap-1">
                  <Button variant="ghost" size="icon" onClick={() => onDetail(supplier)} className="h-8 w-8 text-gray-500 hover:text-indigo-600">
                    <Eye className="h-4 w-4" />
                  </Button>
                  <Button variant="ghost" size="icon" onClick={() => onEdit(supplier)} className="h-8 w-8 text-gray-500 hover:text-indigo-600">
