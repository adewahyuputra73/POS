  ...
  if (customers.length === 0) {
    return (
      <div className="bg-surface rounded-xl border border-border p-12 text-center">
        ...
        <p className="text-text-secondary text-sm">Belum ada data koin pelanggan</p>
      </div>
    );
  }
  ...
  return (
    <div className="bg-surface rounded-xl border border-border overflow-hidden">
      <Table>
        ...
        <TableBody>
          {customers.map((c) => (
            <TableRow key={c.id} className="hover:bg-background/50 transition-colors">
              <TableCell>
                <input
                  type="checkbox"
              ...
              <TableCell className="text-sm text-text-secondary">{c.phone}</TableCell>
              <TableCell className="text-right">
                <div className="flex items-center justify-end gap-1">
                  <Coins className="h-3.5 w-3.5 text-amber-500" />
                  <span className="text-sm font-semibold text-text-primary">{formatNumber(c.totalCoins)}</span>
                </div>
              </TableCell>
              <TableCell className="text-right text-sm text-text-primary font-medium">{formatCurrency(c.coinValue)}</TableCell>
              <TableCell>
