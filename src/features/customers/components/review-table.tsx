  ...
  return (
    <div className="flex items-center gap-0.5">
      {[1, 2, 3, 4, 5].map((star) => (
        <Star
          key={star}
          className={`${size === 'sm' ? 'h-3.5 w-3.5' : 'h-4 w-4'} ${star <= rating ? 'text-yellow-400 fill-yellow-400' : 'text-gray-200'}`}
        />
      ))}
    </div>
  );
...
export function ReviewTable({ reviews, onViewCustomer }: ReviewTableProps) {
  if (reviews.length === 0) {
    return (
      <div className="bg-white rounded-xl border border-border p-12 text-center">
        ...
        <h3 className="text-sm font-medium text-gray-900">Belum ada ulasan</h3>
        <p className="mt-1 text-sm text-gray-500">Ulasan akan muncul setelah pelanggan memberikan feedback</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      {reviews.map((review) => (
        <div key={review.id} className="bg-white rounded-xl border border-border p-5 hover:shadow-sm transition-shadow">
          <div className="flex items-start justify-between mb-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-gray-400">{review.customerPhone}</p>
              </div>
            </div>
            <div className="text-right">
              <StarRating rating={review.rating} size="md" />
              <p className="text-xs text-gray-400 mt-1">{formatDateTime(review.createdAt)}</p>
            </div>
          </div>

          {review.comment && (
            <p className="text-sm text-gray-700 mb-3 pl-[52px]">&ldquo;{review.comment}&rdquo;</p>
          )}
            ...
            <div className="pl-[52px] border-t border-border pt-3 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2">
              {review.questionAnswers.map((qa, i) => (
                <div key={i} className="flex items-center justify-between text-xs bg-gray-50 rounded-lg px-3 py-2">
                  <span className="text-gray-500 truncate mr-2">{qa.questionText}</span>
          ...
          <p className="text-xs text-gray-400 pl-[52px] mt-2">
